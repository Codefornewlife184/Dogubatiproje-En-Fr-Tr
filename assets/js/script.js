$(document).ready(function () {
  ...
  $form.on("submit", function (event) {
    ...
    if ($submitButton.length) {
      $submitButton.prop("disabled", true).text("GONDERILIYOR...");
    }

    renderStatus("loading", "Gönderiliyor", "Mesajınız işleniyor, lütfen bekleyin.");
    ...
    $.ajax({
      ...
      .done(function (response) {
        if (response && response.success) {
          renderStatus(
            "success",
            response.title || "Mesaj gönderildi",
        ...
        renderStatus(
          "error",
          (response && response.title) || "Gönderim başarısız",
          (response && response.message) || "Mesaj gönderilirken bir sorun oluştu."
        );
      })
      .fail(function (xhr) {
        ...
        renderStatus(
          "error",
          response.title || "Gönderim başarısız",
          response.message || "Sunucuya ulaşılırken bir sorun oluştu."
        );
      })
      .always(function () {
        if ($submitButton.length) {
